...
### ✨ Features
*   **UI/Design:**
    *   全面更新 UI 设计，采用玻璃拟态 (Glassmorphism) 风格，并支持深色模式，提升视觉体验。
    *   重构样式系统，从 Material Design 3 迁移至自定义设计令牌 (Design Tokens)，使样式更轻量、灵活。
*   **内容摘要 (Content Summary):**
    *   新增原文/译文切换功能，方便对照阅读。
*   **输入增强 (Input Handling):**
    *   实现“魔术词” (Magic Word) 检测逻辑，支持更智能的输入触发。
    *   增强连续按键检测，引入目标和位置跟踪，减少误触。
*   **交互体验:**
    *   模态框 (Modal) 组件现在支持 `ESC` 键关闭。

### ♻️ Refactors
*   **样式与架构:**
    *   重构 CSS 结构，引入设计令牌，优化代码组织。
    *   简化摘要对话框的定位和尺寸计算逻辑。
*   **核心逻辑:**
    *   优化语言检测逻辑，增加基于字符集 (Charset) 的回退机制。
    *   重构预检规则 (Pre-check rules) 处理，移除冗余代码，提升性能。
    *   改进 DOM 遍历器 (DOM Walker) 中对扩展自身组件的排除逻辑。

### 🐛 Fixes
*   **内容摘要:**
    *   修复 `Ctrl+Enter` 快捷键可能导致的事件冒泡问题。

## [1.5.0] - 2025-10-31

### ✨ 新功能
*   **输入翻译:** 新增对输入框内文本的即时翻译功能，并支持自定义触发方式。
*   **俄语支持:** 新增俄语支持。
*   **AI 增强:** 优化了 AI 翻译的提示词，以获得更可靠的 JSON 格式响应。
*   **语言覆盖:** 增强了语言覆盖逻辑，并开放了所有支持的语言列表。

### ♻️ 优化
*   **性能优化:** 对输入处理、网络请求和 DOM 操作进行了性能优化。
*   **内容摘要:** 优化了内容摘要功能的对话框定位、动画和文本格式。
*   **代码质量:** 重构了部分代码，提高了代码质量和可维护性。

### 🐛 修复
*   修复了输入翻译在特定场景下的语言选项处理问题。
*   修复了内容摘要对话框的定位和滚动问题。
*   改进了连续按键输入的检测逻辑。
...
### ✨ Features
*   **增强的上下文菜单 (Enhanced Context Menu):**
    *   新增语言配置选项
    *   优化样式并支持显示原文
*   **工具提示 (Tooltip):**
    *   实现全新的自定义工具提示系统，取代原生title属性
    *   支持固定/取消固定、拖拽、点击收起等功能
    *   优化UI和国际化支持
*   **AI翻译 (AI Translation):**
    *   增加对标签页上下文的支持
    *   在提示中增加上下文占位符并进行验证
    *   增加prompt占位符的国际化翻译和验证
*   **内容摘要 (Content Summary):**
    *   优化错误处理和UI，增加重试功能
    *   防止摘要界面被意外翻译
*   **语音 (Speech):**
...
*   **AI提示 (AI Prompts):**
    *   调整项目符号范围为3-6
    *   改进JSON解析，支持Markdown代码块
    *   优化AI提示词处理和格式
*   **语言检测 (Language Detection):**
    *   使用franc库替代基于正则表达式的规则，集中处理语言检测和配置
*   **UI/UX:**
    *   优化加载和错误状态的CSS动画
    *   修复弹窗设置加载问题，确保规则来源一致
    *   优化弹窗显示模式的更新，实现即时响应
...
### 🐛 Fixes
*   **设置 (Settings):**
    *   为规则启用复选框添加唯一ID和正确的标签关联
    *   修复域名规则模态框中引擎显示和默认值处理的问题

### 💄 Styles
*   **UI/UX:**
    *   引入Material Design 3风格，优化整体界面
...
### 🏗️ Build
*   **依赖 (Dependencies):**
    *   增加franc.bundle.mjs库的支持，并提供版本检查和自动下载
