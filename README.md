...
- **AI 助手 (总结与对话)**: 在支持（需主动设置域名配置）的页面上，会出现一个悬浮的“总结”按钮。点击后，AI 会自动总结页面核心内容，并开启一个对话窗口，您可以基于页面内容或任意话题与 AI 进行持续对话。
...
- **内容过滤 (内置预检)**: 发送翻译前自动跳过 URL、邮箱、纯数字/符号、代码块标记，以及已是目标语言的文本（内置规则，无需配置）。
- **键盘快捷键**:
  - `Alt+S`: 快速翻译您选中的文本。
...
- **现代化 UI 设计**：采用全新的玻璃拟态 (Glassmorphism) 设计风格，支持深色模式，提供更现代、美观且舒适的视觉体验。
...
1.  **启用功能**: AI 助手功能需要在“域名规则”中为特定网站启用。
2.  进入“选项”页面，找到或创建一个针对目标网站（如 `en.wikipedia.org`）的域名规则。
3.  在规则设置中，找到 **“总结设置” (Summary Settings)** 部分。
4.  勾选 **“启用总结功能” (Enable Summary Feature)**。
5.  填写 **“主体内容选择器” (Main Body Selector)**，这是一个 CSS 选择器，用于告诉扩展页面的哪个部分是需要总结的核心内容（例如，对于维基百科，可以是 `#bodyContent`）。
6.  选择一个用于总结和对话的 **AI 模型**。
7.  保存设置后，在目标网站的页面上，您应该能看到一个悬浮的“总结”按钮。点击它即可开始使用。

    - **交互**: 点击按钮后，将自动进行总结。总结完成后，您可以在弹出的对话框中继续提问，AI 将基于上下文进行回答。支持一键切换显示原文/译文，方便对照阅读。
    - **拖动**: 您可以按住并拖动总结按钮到页面的任意位置。
...
- **可配置项**:
  ...
  - **总结设置 (Summary Settings)**: 为该域名启用 AI 总结与对话功能，需要配置内容主体选择器和使用的 AI 模型。
...
在文本发送到翻译引擎之前，Foxlate 使用**内置**过滤器跳过：URL、邮箱地址、纯空白/数字/标点、代码块围栏、单个 emoji，以及语言检测判定为已是目标语言的文本。当前版本**不支持**用户自定义正则预检规则；站点级控制请使用「排除选择器」与域名规则。
