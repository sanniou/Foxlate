...
- **集成原生翻译 API**: 探索并集成浏览器（如 Chrome 138+）内置的 `Translator API` 和 `Language Detector API`，以实现更轻量、更快速的离线翻译体验。
- **PDF 文件翻译**: 支持在浏览器中直接打开和翻译本地或在线的 PDF 文件。
- **独立翻译面板**: 提供一个独立的翻译面板或窗口，用户可以在其中输入文本进行翻译，类似于一个完整的翻译应用。
- **更多翻译引擎**: 增加对更多流行翻译服务的支持。
- **UI/UX 改进**: 持续打磨设置页面和翻译结果的交互体验。
